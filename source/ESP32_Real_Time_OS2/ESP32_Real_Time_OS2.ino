...
void setup() 
{
  ...
  { 
    ...
    { 
      ...
      { 
        relay1RealState = false; digitalWrite(SW1_RELAY_PIN, LOW); 
      } 
    }
  };
  ...
  thing["d_sun"] << [](pson& in){ if(in.is_empty()) in = cfg.days[0]; else { cfg.days[0] = in; saveConfig(); } }; // Sunday
  thing["d_mon"] << [](pson& in){ if(in.is_empty()) in = cfg.days[1]; else { cfg.days[1] = in; saveConfig(); } }; // Mon
  thing["d_tue"] << [](pson& in){ if(in.is_empty()) in = cfg.days[2]; else { cfg.days[2] = in; saveConfig(); } }; // Tue
  thing["d_wed"] << [](pson& in){ if(in.is_empty()) in = cfg.days[3]; else { cfg.days[3] = in; saveConfig(); } }; // Wed
  thing["d_thu"] << [](pson& in){ if(in.is_empty()) in = cfg.days[4]; else { cfg.days[4] = in; saveConfig(); } }; // Thu
  thing["d_fri"] << [](pson& in){ if(in.is_empty()) in = cfg.days[5]; else { cfg.days[5] = in; saveConfig(); } }; // Fri
  thing["d_sat"] << [](pson& in){ if(in.is_empty()) in = cfg.days[6]; else { cfg.days[6] = in; saveConfig(); } }; // Sat
